...
import Title from '../../ui/Title/Title';import Paper from '../../ui/Paper/Paper';
...
const GroupView: FC<ViewWithModel<GroupModel>> = (props) => {
  ...
  const { cardIds, page, isEnd } = InstanceTaskManagerStore;


  useUpdate(didUpdate, [cardIds.length, model.name, model.items.length]);

  return (
    <Paper className={styles.group}>
      <Row className={styles.row}>
        ...
        <Button className={makeClass([styles.moreButton, isEnd ? styles.noMore : ''])} callback={() => !isEnd ? InstanceTaskManagerStore.getTodo() : {}}>{`${isEnd ? 'No' : ''} More...`}</Button>
      </Row>
    </Paper>
  );
};
